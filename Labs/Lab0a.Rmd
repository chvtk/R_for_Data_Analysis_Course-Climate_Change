---
title: "Lab 0a - Introduction to R and RStudio"
output:
...
The goal of this lab is to introduce you to R and RStudio, which you'll be using throughout the course both to analyze data related to climate change and come to informed conclusions.  To straighten out which is which: R is the name of the programming language itself and RStudio is a convenient interface.

As the labs progress, you are encouraged to explore beyond what the labs dictate; a willingness to experiment will make you a much better programmer.  Before we get to that stage, however, you need to build some basic fluency in R. Today we begin with the fundamental building blocks of R and RStudio: the interface, reading in data, and basic commands.

![Image of RStudio Interface](../../R_interface.png)


Above is an image of the RStudio interface. The panel in the upper right contains your **workspace** as well as a history of the commands that you've previously entered.  Any plots that you generate will show up in the panel in the lower right corner.

The panel on the left is where the action happens.  It's called the **console**.  Everytime you launch RStudio, it will have the same text at the top of the console telling you the version of R that you're running.  Below that information is the **prompt** (marked by the blue arrow).  As its name suggests, this prompt is really a request- a request for a command.  Initially, interacting with R is all about typing commands and interpreting the output. These commands and their syntax have evolved over decades (literally) and now provide what many users feel is a fairly natural way to access data and organize, describe, and invoke statistical computations.

To get you started, we are going to explore another component of R studio - the ability to create a reproducible data analysis through creation of an R markdown document. Click on File --> New File --> R Markdown. This creates an R Markdown file (.rmd), which allows inclusion of all text, R code, and metadata for a fully documented and reproducible analysis. You will be analysing and writing all of your reports in R markdown. 

Give your new markdown file a name and author, and select HTML as the output format. Note that presentations can also be made using R markdown. Once you've done this, **stop** and give your instructor some attention as she walks you through some other R markdown info. If you're waiting for classmates to wrap up, look over the info contained in your new markdown file.  

Ok - you're back! Pop onto the Moodle page and download the Arbuthnot dataset - make sure to save it in the THAWSPACE drive on the computer. Type the following command (one after the hashtag) in a new R chunk and then run the command (command key + enter OR the "Run" button on the top right of the Rmarkdown document). 

```{r, eval=TRUE}
arbuthnot = read.csv("/Users/koechlic/Desktop/arbuthnot.csv")
#arbuthnot = read.csv("/Volumes/THAWSPACE/arbuthnot.csv")
```
After reading the following information, add in some markdown or comment to summarize what your command is doing. Continue to comment your actions throughout the tutorial.

This command instructs R read in a data set file, the Arbuthnot baptism counts for boys and girls, and save it in a variable called "arbuthnot". You should see that the workspace area in the upper righthand corner of the RStudio window now lists a data set called `arbuthnot` that has 82 observations of 3 variables. As you interact with R, you will create a series of objects. Sometimes you load them as we have done here, and sometimes you create them yourself as the byproduct of a computation or some analysis you have performed. Note that if you are trying to recreate this lab on your own computer, you will need to change the **path** (the information in quotation marks in the read.csv function) to the file - for example, if you save the file on your own personal Desktop, the file path may alter to "Desktop/Arbuthnot.csv". If you are running a Windows operating system, you will need to place C: before the path name : ex. "C:Desktop/Arbuthnot.csv". The path to the file all depends on the location of your Rmarkdown file - Rmarkdown documents use the directory in which they reside as their base directory.  

## The Data: Dr. Arbuthnot's Baptism Records

The Arbuthnot data set refers to Dr. John Arbuthnot, an 18<sup>th</sup> century physician, writer, and mathematician.  He was interested in the ratio of newborn boys to newborn girls, so he gathered the baptism records for children born in London for every year from 1629 to 1710.  We can take a look at the data by typing its name into a new R chunk. 
...
1.  What command would you use to extract just the counts of girls baptized? Try it and record your code and annotation!
...
You should see a help file in the lower right panel. Help files all have the similar format. Browse through and see what information is provided.
...
In the previous few pages, you recreated some of the displays and preliminary analysis of Arbuthnot's baptism data. Your assignment involves repeating these steps, but for present day birth records in the United States.  Load up the present day data from the Moodle page.

Store these data in a data frame called `present`.
...
Once you are done with the "On your own", knit a markdown document (using the "Knit" icon at the top of the R markdown console). Knitting renders the R markdown into an HTML document - make sure to marvel at your ability to make a nice looking document! Before you finish this tutorial, save a version of your R markdown document in your Google Drive. 
