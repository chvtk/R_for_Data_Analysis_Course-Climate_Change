...
```{r}
survey = read.csv("/Users/koechlic/Desktop/Clim_Change_Opinion_Survey.csv")
...
To extract rows 10 through 16 from the **survey** data frame we use the `slice` function. This replaces the indexing we used in Wednesday's lab.
```survey[1:10,]
      ```
...
Think about how you could apply each of the the dplyr commands on either the `survey`, `Arbuthnot`, or `present` datasets. Then - try it out!
