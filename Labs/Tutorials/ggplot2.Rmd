...
AmesHousing <- read.csv("/Users/koechlic/Desktop/AmesHousing.csv")
head(AmesHousing)
```
...
By just looking at the above graph, would you classify the distribution of house sale prices as normal? 
...
ggplot(data=AmesHousing) +                         
      geom_histogram(mapping = aes(SalePrice/100000), 
          breaks=seq(0, 7, by = 0.5), col="red", fill="lightblue") + 
...
In the code below we create three scatterplots of the log of the above ground living area by the log of sales price. Note that log() when used in R, refers to the natural logarithm (also known as ln, or the log with base e). To specify log base 10, the command log10() is used. Any other log can be specified by logb([numeric value or vector], base = [base to use]).

```{r}
ggplot(data=AmesHousing, aes(x=log(Gr.Liv.Area), y=log(SalePrice)) ) +      
  geom_point(shape = 3, color = "darkgreen") +                                     
  geom_smooth(method=lm,  color="green") +                  
  labs(title = "Figure 10", x = "log(Sale Price)", y = "Aboveground living area (sq.ft.)" )

 
ggplot(data=AmesHousing) + 
  geom_point(aes(x=log(Gr.Liv.Area), y=log(SalePrice), color=Kitchen.Qual), size=1) + 
  geom_smooth(aes(x=log(Gr.Liv.Area), y=log(SalePrice), color=Kitchen.Qual), 
          method=loess, size=2) +                        
  labs(title = "Figure 11", y="log(Sale Price)", x = "Aboveground living area (sq.ft.)", color = "Kitchen\nQuality") + 
  scale_color_discrete(labels = c("Excellent", "Fair", "Good", "Poor", "Typical"))

ggplot(data=AmesHousing) +
  geom_point(mapping = aes(x=log(Gr.Liv.Area), y=log(SalePrice), color=Kitchen.Qual), alpha = 0.3) +
  geom_smooth(mapping = aes(x=log(Gr.Liv.Area), y=log(SalePrice), color=Kitchen.Qual), 
      method=lm, se=FALSE, fullrange=TRUE) +                             
  facet_grid(. ~ Fireplaces) +                      
  labs(title = "Figure 12", y="log(Sale Price)", x = "Aboveground living area (sq.ft.)", color = "Kitchen\nQuality") + 
   scale_color_discrete(labels = c("Excellent", "Fair", "Good", "Poor", "Typical"))
```
...
*  When assigning fixed characteristics, the commands occur outside the `aes`, as in Figure 10, `color="green"`. When characteristics are dependent on the data, the command should occur within the `aes`, such as in Figure 11 `color=Kitchen.Qual`.
* The command `alpha`, used in Figure 12, allows adjustment of color or fill transparency. It can be extremely useful when working with a lot of data points. 
*  Excellent resource for working with those pesky legends : http://www.cookbook-r.com/Graphs/Legends_(ggplot2)/


In the above examples, only a few `geoms` are listed. The [ggplot2 website](http://docs.ggplot2.org/current/) lists each `geom` and gives detailed examples of how they are used. Also the ggplot2 cheatsheet provides great visualization of these geoms. 
...
ggplot(AmesHousing2, aes(x = Fireplace2, y = SalePrice, color = Paved.Drive)) +
  ...
  coord_flip()+ 
  labs(title="Housing Prices in Ames, Iowa", x = "Number of Fireplaces", y = "Sale Price ($)") +
...
In order to complete this activity, you will need to go through the `dplyr` tutorial and use the `dplyr` package to manipulate the dataset before making any graphics.
